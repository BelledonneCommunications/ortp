...
ortp_message("%s -- DEBUG DEBUG Do change %0d is RTP %0d Socket rtp %0d rtcp %0d packets received: rtp %0llu rtcp %0llu only at start %0d Trying to switch %s destination from %s to %s for session [%p]"
			,__func__, do_address_change, is_rtp, session->rtp.gs.socket, session->rtcp.gs.socket, (unsigned long long)rtp_session_get_stats(session)->packet_recv, (unsigned long long)rtp_session_get_stats(session)->recv_rtcp_packets, only_at_start
			   , socket_type
			   , current_ip_address
			   , new_ip_address
			   , session);
