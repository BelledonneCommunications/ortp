...
void dblk_unref(struct datab * data){
	atomic_int previous_ref(atomic_fetch_sub_explicit(static_cast<atomic_int*>(data->db_ref), 1, memory_order_release));
	if (previous_ref == 1){
		if (data->db_freefn!=NULL)
			data->db_freefn(data->db_base);
